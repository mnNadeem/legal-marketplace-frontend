...
const CheckoutInner: React.FC = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <PaymentElement />
      <Button type="submit" loading={submitting} disabled={!stripe || submitting}>Pay</Button>
    </form>
  );
};
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const quote = (c.data.quotes || []).find((qq: any) => qq.id === quoteId);
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        <Card>
          ...
          {loading ? (
            ...
            <p className="text-gray-600">You are paying {amount ? `$${amount}` : 'the selected amount'} for this case.</p>
          )}
        </Card>

        <Card>
          {!STRIPE_PK ? (
            <div className="text-sm text-red-600">Missing Stripe publishable key in env.</div>
          ) : loading || !clientSecret ? (
            <div className="flex justify-center py-6"><LoadingSpinner size="md" /></div>
          ) : (
            stripePromise ? (
              <Elements stripe={stripePromise} options={{ appearance, clientSecret: clientSecret || '' } as StripeElementsOptions}>
                <CheckoutInner />
              </Elements>
            ) : (
              <div className="text-sm text-red-600">Stripe failed to initialize.</div>
            )
          )}
        </Card>
      </div>
