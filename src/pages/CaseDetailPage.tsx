...
const CaseDetailPage: React.FC = () => {
  ...
  const isClientOwner = user?.role === 'client' && user?.id === caseData?.clientId;
  ...
  const handleDownload = async (fileId: string) => {
    try {
      ...
      if (url) {
        ...
        window.open(`/files/secure/${fileId}?token=${encodeURIComponent(token)}`, '_blank', 'noopener,noreferrer');
      } else {
    ...
    return (
      <DashboardLayout>
        <Card className="text-center py-12">
          ...
          <p className="mt-1 text-sm text-gray-500">The requested case does not exist.</p>
        </Card>
      </DashboardLayout>
    );
  }
              ...
              <ul className="divide-y divide-gray-200">
                {caseData.files.map((f) => (
                  <li key={f.id} className="py-3 flex items-center justify-between">
                    ...
                    {(isClientOwner || (user?.role === 'lawyer' && caseData.status === 'engaged' && caseData.quotes?.some(q => q.lawyerId === user.id && q.status === 'accepted'))) ? (
                      <Button size="sm" variant="outline" onClick={() => handleDownload(f.id)}>Download</Button>
                    ) : (
            ...
            <Card>
              <div className="flex items-center justify-between mb-4">
                ...
                {isClientOwner && caseData.status === 'open' && (
                  <span className="text-sm text-gray-600">{caseData.quotes?.length || 0} received</span>
                )}
              </div>
              ...
              {caseData.quotes && caseData.quotes.length > 0 ? (
                <div className="space-y-3">
                  {caseData.quotes.map((q) => (
                    <div key={q.id} className="p-4 border rounded-md flex items-center justify-between">
                      <div className="space-y-1">
                        <div className="flex items-center space-x-6 text-sm text-gray-600">
                          ...
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${q.status === 'accepted' ? 'bg-green-100 text-green-800' : q.status === 'rejected' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'}`}>
                            {q.status}
                          </span>
                        </div>
                        {q.note && <p className="text-sm text-gray-700">{q.note}</p>}
                      </div>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        {isClientOwner && caseData.status === 'open' && q.status === 'proposed' && (
                          <Button
                            size="sm"
                            onClick={() => navigate(`/cases/${caseData.id}/checkout/${q.id}`)}
                          >
                            Accept & Pay
                          </Button>
                        )}
